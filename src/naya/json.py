...
def __readchar(stream):
    return str(stream.read(1),'uft-8')
...
def parse_next(token_stream, first_token=None):
    """
        Parse the next object of array in the stream.
        Returns : a tuple of : the object and the following token (or None if end of stream).
    """ 
    if first_token == None: first_token=next(token_stream)
    obj, token_type, token = __parse(token_stream, first_token)
    if token_type == None: 
        return obj, None
    else:
        return obj, (token_type, token)
